...
class Entry {
	...
	public function entry_save_location( $fields, $entry, $form_id, $form_data ) {
		...
		if ( empty( wpforms()->get( 'process' )->entry_id ) ) {
			return;
		}
		...
		if ( $this->has_entry_geolocation_smart_tag( $form_data ) ) {
			...
			if ( $loc ) {
				wpforms()->get( 'entry_meta' )->add(
					[
						'entry_id' => absint( wpforms()->get( 'process' )->entry_id ),
		...
		wpforms()
			...
			->async()
			->params(
				absint( wpforms()->get( 'process' )->entry_id ),
				absint( $form_id ),
				wpforms_get_ip()
			)
			->register();
	...
	private function has_entry_geolocation_smart_tag_in_notifications( $form_data ) {
		...
		if ( ! empty( $form_data['settings']['notifications'] ) ) {
			foreach ( $form_data['settings']['notifications'] as $notification ) {
				...
				if ( isset( $notification['entry_csv_attachment_enable'], $notification['entry_csv_attachment_entry_information'] ) && in_array( 'entry_geolocation', $notification['entry_csv_attachment_entry_information'], true ) ) {
					return true;
				}
			}
		}
	...
	public function add_print_page_styles( $entry, $form_data ) {
		...
		echo '<style>#print:not(.wpforms-preview-mode-location) .wpforms-field-location {display: none;}</style>';
	}
